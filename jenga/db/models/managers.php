...
abstract class ModelManager {
	...
	public function get($conditions) {
		...
		if($qs[0] == null)
			throw new \Exception('Could not pull object from a ' .  $this->model_name . ' Query.');
		return $qs[0];
	}
		...
		foreach($reflection_model->fields as $field_name => $field) {
			...
			if($field[0] == f\ForeignKey) {
				...
				if(!$can_be_null) {
					...
					if(isset($model->$field_id_name))
						$value = $model->$field_id_name;
					else if(isset($model->$field_name->id))
						$value = $model->$field_name->id;
					else
						$value = null;
				}
				...
				if(isset($model->$field_name) && is_bool($model->$field_name))
					$value = $model->$field_name;
